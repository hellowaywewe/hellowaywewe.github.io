...
正如上文《APT镜像软件源更新及格式解析》所述，apt是Ubuntu的包管理工具，为了加快下载速度，我们在使用apt安装软件时，常常需要将默认的软件源替换成
国内的镜像源。除此之外，有些软件（如：前面所说的ppa软件）还需要先获取该软件包的GPG公钥，才能成功安装。关于[GPG加解密知识学习][0]的入门不作为
本文重点，如需了解，可自行查阅相关资料。
...
2. 更换Kubernetes国内源，这里通过[阿里云镜像站][1]查询Kubernetes可用安装包，在/etc/apt/sources.list.d/目录下创建kubernetes.list文件，
并将所需源内容写入该文件保存即可。
...
Reference links:
[0] https://www.ruanyifeng.com/blog/2013/07/gpg.html?tdsourcetag=s_pctim_aiomsg "GPG加解密知识学习"
[1] https://developer.aliyun.com/mirror/ "阿里云镜像站"
